...
def to_csv_file(filename, headers, rows, delimiter=",", new_line="\n"):
    txt = ""
    if headers is not None:  # Проверить предоставлены ли заголовки, если да, то записать их в строку
        txt += delimiter.join(headers) + new_line

    if rows is not None:  # Проверить предоставлены ли данные, если да, то записать построчно в строку
        txt += new_line.join([delimiter.join(row) for row in rows])
